...
- 最新snapshot `snap-ee986754-8243-4c2b-b359-8ab415d5e61c` で index が作成された。
  - `snapshot_payload_index`: 1
  - `snapshot_payloads`: 16002
  - `large_object_chunks`: 10169
- `mj fsck --since <latest-created-at> --sample 10 --timeout-secs 90 --progress` は `source=index` でscope構築し成功。
- `mj cache stat --metadata` は payload / metadata cache candidates ともに 0。
- `mj sync status` は `local_current == remote_current == snap-ee986754-8243-4c2b-b359-8ab415d5e61c`、queued uploads 0、sync lock なし。
- `mj daemon status` は pending journal 0、queued uploads 0。
- `mj status --no-pager` は state apparent size 40.1 MiB、disk 44.3 MiB、trees 0 B。
