...
自動pruneは保持期間にかかわらず、remote payloadが既に欠損して復元不能になったcurrent以外の
snapshot metadataも削除する。これにより、復元不能な履歴だけが残ってfull fsckを継続的に
失敗させる状態を防ぐ。S3 backendでは現在hostのprefixだけを判定対象にする。
...
`mj sync status --deep --sample` は16件以下の小標本では対象keyをHEAD probeし、それより多い場合は
現在hostのpayload prefixを並列LISTしてローカル照合する。これにより、大きいsampleでobjectごとの
GCS latencyとHEAD課金を繰り返さない。
...
通常の `mj remote fsck` は metadata health を確認する。compact head を持つ S3 remote では
current head と gc mark を正とし、過去の GC tombstone 全件取得は省略するため、履歴が長い
host でも一定時間で完了する。全 tombstone を含む履歴監査は deep metadata check の対象である。
quick mode には既定 60 秒の全体 timeout があり、phase progress は stderr に出力される。
...
# quick metadata audit の timeout を変更する場合
mj remote fsck --timeout-secs 120
```
...
S3/GCS 互換 remote では、最新 head 情報を `<host-prefix>/head.cbor.zst.enc`
